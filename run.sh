...
sudo docker run --restart=always --name dictionary-service -p 8000:8000 -v `pwd`/Service/:/opt/Service -d dictionary-service 
...
sudo docker run --restart=always --name dictionary-front-end -p 8080:8080 -v `pwd`/Front-end/:/opt/Front-end  -d dictionary-front-end  